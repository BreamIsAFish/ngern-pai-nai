...
export default function App() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (nextStatus.signedIn && nextStatus.sheetReady && !nextStatus.schemaResetRequired) {
        ...
        if (visualState?.startsWith('edit-transaction') && rows[0]) setEditing(rows[0])
      }
    } catch (caught) {
  ...
  const save = (input: TransactionInput) => {
    const original = editing ?? undefined
    const timestamp = new Date().toISOString()
    const optimistic: Transaction = {
      ...input,
      id: original?.id ?? `optimistic-${Date.now()}-${Math.random().toString(36).slice(2)}`,
      createdAt: original?.createdAt ?? timestamp,
      updatedAt: timestamp,
    }
    pendingTransactionIds.current.add(optimistic.id)
    setError(undefined)
    setEditing(undefined)
    if (screen === 'search') setScreen('home')
    setTransactions((current) => {
      const withoutChanged = current.filter((item) => item.id !== optimistic.id)
      return transactionLocalMonth(optimistic) === monthRef.current ? [optimistic, ...withoutChanged] : withoutChanged
    })

    void (async () => {
      try {
        const result = original
          ? await bridge.request('transactions.update', { id: original.id, sourceUtcMonth: utcMonthForTransaction(original), transaction: input })
          : await bridge.request('transactions.create', { transaction: input })
        const persisted = mapTransaction(result)
        setTransactions((current) => {
          const withoutOptimistic = current.filter((item) => item.id !== optimistic.id && item.id !== persisted.id)
          return transactionLocalMonth(persisted) === monthRef.current ? [persisted, ...withoutOptimistic] : withoutOptimistic
        })
      } catch (caught) {
        setTransactions((current) => {
          const rolledBack = current.filter((item) => item.id !== optimistic.id)
          return original && transactionLocalMonth(original) === monthRef.current ? [original, ...rolledBack] : rolledBack
        })
        setError(caught instanceof BridgeRequestError && caught.code === 'DUPLICATE_RECEIPT'
          ? 'รายการนี้ซ้ำกับรายการที่มีอยู่'
          : caught instanceof Error ? caught.message : 'Could not save the transaction.')
      } finally {
        pendingTransactionIds.current.delete(optimistic.id)
      }
    })()
  }

  const remove = (transaction: Transaction) => {
    pendingTransactionIds.current.add(transaction.id)
    setError(undefined)
    setEditing(undefined)
    if (screen === 'search') setScreen('home')
    setTransactions((current) => current.filter((item) => item.id !== transaction.id))

    void bridge.request('transactions.delete', { id: transaction.id, utcMonth: utcMonthForTransaction(transaction) })
      .then(() => setTransactions((current) => current.filter((item) => item.id !== transaction.id)))
      .catch((caught) => {
        setTransactions((current) => transactionLocalMonth(transaction) === monthRef.current && !current.some((item) => item.id === transaction.id)
          ? [transaction, ...current]
          : current)
        setError(caught instanceof Error ? caught.message : 'Could not delete the transaction.')
      })
      .finally(() => pendingTransactionIds.current.delete(transaction.id))
  }

  const editTransaction = (transaction: Transaction) => {
    if (!pendingTransactionIds.current.has(transaction.id)) setEditing(transaction)
  }
  ...
  return (
    <div className={`phone-canvas screen-${screen}`} data-visual-ready="true">
      ...
      {receiptSelection && <ReceiptImportModal onClose={() => { setReceiptSelection(undefined); void loadMonth(month) }} onEdit={editTransaction} onTransactionAdded={(transaction) => { if (transactionLocalMonth(transaction) === month) setTransactions((current) => [transaction, ...current.filter((item) => item.id !== transaction.id)]) }} onTransactionDeleted={(id) => setTransactions((current) => current.filter((item) => item.id !== id))} selection={receiptSelection} />}
      {editing !== undefined && <TransactionForm categories={categories} initialCategoryPickerOpen={visualState?.startsWith('add-expense-picker')} initialType={entryType} onCancel={() => setEditing(undefined)} onDelete={editing ? () => remove(editing) : undefined} onManageCategories={() => { setEditing(undefined); setScreen('categories') }} onManageTags={() => { setEditing(undefined); setTagManagerAdding(true); setScreen('tags') }} onSubmit={save} tags={tags} transaction={editing ?? undefined} />}
    </div>
  )
}
