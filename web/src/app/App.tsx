...
export default function App() {
  ...
  const signOut = async () => {
    setBusy(true)
    try { setStatus(await bridge.request('google.signOut', {})); setTransactions([]); setScreen('home') }
  ...
  return (
    <div className={`phone-canvas screen-${screen}`} data-visual-ready="true">
      ...
      {screen === 'profile' && <SettingsPage busy={busy} onHome={() => setScreen('home')} onManageSheet={() => setScreen('sheet')} onManageCategories={() => setScreen('categories')} onManageTags={() => { setTagManagerAdding(false); setScreen('tags') }} onSignOut={() => void signOut()} status={status} />}
      ...
      {screen === 'summary' && <SummaryPage initialView={visualState === 'summary-categories' ? 'list' : 'chart'} latestMonth={localDateValue().slice(0, 7)} loading={monthLoading} month={month} monthPickerOpenInitially={visualState === 'summary-month-picker'} onBack={() => setScreen('home')} onChangeMonth={changeMonth} transactions={transactions} />}
